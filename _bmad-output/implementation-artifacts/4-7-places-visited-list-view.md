...
- 2026-06-24 — Enhancement (Simon request): bare regions are now NAVIGABLE. Added `scripts/build-centroids.mjs` (Natural Earth → representative point per ISO code, keyed exactly like the marks/tiles) → bundled `features/places/region-centroids.json` (4,552 entries: 239 countries + 4,313 regions) + a `regionCentroid(code)` lookup. The Places list now flies the map to a bare region's centroid (it was close-only); revives the bare region as a button (replacing the review's label-only). No migration, no production write, works offline. tsc/lint/build green.
