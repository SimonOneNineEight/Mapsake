...
- **Returning-user sign-in + map merge (Story 2-3) — STILL OPEN.** 2-2 now TELLS the returning user "this email already has an account, sign in with your original method", but the actual mechanism to sign INTO that existing account (not `updateUser`/`linkIdentity`, which both fail with `email_exists` for a taken email — needs `signInWithOtp` / a real sign-in path) and the policy for what happens to the current device's anonymous map (merge / import-prompt / discard — see the industry-pattern discussion) are the heart of Story 2-3. The message is the floor; the consolidation is the story.
