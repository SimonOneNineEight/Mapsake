...
- **~~OAuth error feedback unconsumed + already-linked copy is generic~~ — DONE in 2-2 (2026-06-25).** The error surfacing was pulled forward at Simon's request: `/auth/callback` branches on `error_code` (`email_exists`/`identity_already_exists` → `/?auth_error=existing`; `access_denied` → silent), and `account-sheet.tsx` reads the query flag on load and opens the sheet with a calm zh-TW notice. **Correction to the earlier manual-verify note:** a probe proved Supabase PKCE delivers `error_code` in the **query** at `/auth/callback` (the hash is only a mirror, and is unreliable across the redirect on Safari/Firefox) — so we read it server-side from the query, NOT from `window.location.hash`. The earlier "parse the hash" guidance was wrong; the cross-browser-safe source is the query.
- **Returning-user sign-in + map merge — moved to Story 2.7 (carved from 2-3, 2026-06-25).** 2-2 TELLS the returning user "this email already has an account, sign in" and 2-3 keeps that message as the floor, but the real mechanism to sign INTO the existing account (not `updateUser`/`linkIdentity`, which fail with `email_exists` — needs `signInWithOtp`/`signInWithOAuth`) and the import-prompt merge of the device's anon map (the app's first SECURITY DEFINER/service-role surface: re-parent `region_marks`/`pins`/`photos` to the target uid + move `pin-photos` storage objects + conflict rules + cache reset) are now **Story 2.7**. Simon's call: prompt + in-place claim ship in 2-3; the merge is its own story given its risk for a rare second-device edge.

## Epic 2 retrospective — carry-forwards (2026-06-25)

Epic 2 (Keep your map) v1 complete: 2-1..2-7 done; 2-8 deferred. Carry-forwards:

- **Manual auth-config debt (highest open risk):** the real end-to-end auth round-trips are UNVERIFIED, gated on Simon's Supabase dashboard config — (a) Google: Cloud OAuth client + provider + **Manual Linking ON** + `/auth/callback` allow-listed; (b) the **"Magic Link" email template** for 2-7's `signInWithOtp` sign-in (point it at `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=magiclink`, distinct from 2-1's templates). All code lands against these; verify before any real-user/multi-device use. 2-1 email magic-link IS verified end-to-end; Google + returning-sign-in are not.
- **Story 2-8 (cross-account anon-map merge):** the app's first SECURITY DEFINER/service-role surface — re-parent `region_marks`/`pins`/`photos` + move `pin-photos` storage objects from the anon uid to the target account, import-prompt UX, conflict-safe rules, cache reset. Backlog; recommended approach in the 2-3 deferred note. Needs a gated migration.
- **Auth error-code distinction:** `/auth/callback` collapses `email_exists` vs `identity_already_exists` into one `?auth_error=existing`. Splitting them would enable a Google one-click sign-in fast-path for Google-created accounts (2-7 deferred this; email-OTP is the universal recovery today) and a more precise message.
- **Orphaned anonymous users accumulate:** 2-7 sign-in leaves the device's anon user + its rows orphaned (not destroyed). Confirm Supabase's anonymous-user retention so a days-later 2-8 merge still finds them; add a cleanup story when 2-8 lands.
- **Signed-in e2e gap (recurring):** the shared test harness is anon-only, so signed-in paths are NOT e2e-covered — the 2-3 prompt-not-shown-to-signed-in, the 2-6 export trigger, 2-7 uid-continuity, and 2-4 AC1 (same map on device 2). A permanent test account + a two-session RLS integration test would close this (fits 6-5 CI hardening).
- **Polish deferred to Epic 6:** the save-status auto-dismiss (2-5), the durable-write outbox/PowerSync + field-level-merge conflict model (post-v1), the export binaries/zip + relocating export into the real Settings surface (6-3), and i18n externalization of the inline zh-TW strings (6-1).
- **Per-user delivery-time honoring (Story 5-6 fast-follow):** 5-6 STORES `profiles.notif_time` (the delivery-time picker) but the 5-3 cron uses a single fixed Taiwan-evening hour (`0 11 * * *` UTC) and does NOT read it (Simon chose "store-only"). To honor it: run the cron HOURLY and send to users whose `notif_time` hour matches the current hour (assume the zh-TW audience is UTC+8 — or add a per-user timezone column for correctness). Until then the time picker is a stored preference, not yet acted on.

## Epic 5 retrospective carry-forwards (epic-level cross-story audit, 2026-06-25)
- **[HIGH] Tier-2 EXIF anniversary is dead:** nothing writes `pins.exif_taken_at` — the photo pipeline writes `photos.taken_at` per-photo but never rolls it up. The 5-2 engine reads `exif_taken_at` for tier-2 + the photos tiebreaker, so both are inert (users with old-trip photos but no typed date get tier-3 "N 年前加入" instead of tier-2 "N 年前的今天"). Fix: denormalize `MIN(photos.taken_at)` → `pins.exif_taken_at` on photo insert/delete (data layer or DB trigger), or have the sender read it. Epic 6 / pre-launch fast-follow.
- **[MED] "N more" count can disagree:** the push count requires a true anniversary (`yearsAgo≥1`); the landing chip's `memoriesSharingDay` matches month-day with no year constraint. Plus a rediscovery push (day-less) lands tier-blind and builds a false same-day cohort on the rediscovered pin's month-day. Fix: carry the tier/count through the deep link (`?pin=id&...`), or only build the landing cohort when the target is an anniversary of today.
- **[MED] Two notification controls contradict:** `EnableNotifications` (device permission) + `NotificationSettings` (`notif_enabled`, default true) can show "enable" AND "on" at once. Fold into a single coherent state in the real Settings surface (6-3).
- **[MED/LOW] web-push robustness:** a 403 (mismatched VAPID pair) is treated as transient → not pruned, ledger never stamps → retries daily forever with only console output. Treat 403 as loud. And the rediscovery cadence is effectively 31 days not 30 (timestamp-vs-date-string `daysBetween`) — slice `last_rediscovery_at` to its date first.
- **[INFO] Deploy ordering is a silent-failure trap:** `NEXT_PUBLIC_VAPID_PUBLIC_KEY` is build-time inlined (set env → THEN build; rotation needs a rebuild); type regen must run AFTER the migrations; `CRON_SECRET` must be in the Production scope. Full runbook in `epic-5-retrospective.md`.
- **[INFO] usePin docstring stale** (says a dedicated getPin is needed for re-live; 5-4 correctly uses the list cache); the `?pin=` scrub runs before the pin resolves, so a cold-start fetch error has no retry path.

## Story 6-1 i18n deferred (2026-06-25)
- **Lib/server strings not yet in the catalog (externalize when `en` lands):** `features/notifications/lib/push-copy.ts` (the 9 server-generated push-copy strings — needs the cron route to pass an injected `getTranslations`, see Epic 5 retro), and `features/places/lib/build-places.ts`'s `其他` fallback label for the "??" (no-country) bucket (a pure lib; would take an injected label from places-panel). Both are non-component lib strings, consistent with 6-1's "UI component strings" scope. `messages/en.json` + the locale switch are the post-v1 follow-up.
- **Native-voice polish:** all zh-TW strings were externalized VERBATIM into `messages/zh-TW.json` (one namespaced file). Polishing the voice is now a single-file review (no code changes) whenever Simon wants it.

## Post-launch hardening surfaced during prod testing (2026-06-26)
- **[MED] Base-map PMTiles is still precached (28.9MB wasted per install):** Serwist auto-globs `public/` into the precache manifest, so `public/tiles/boundaries.pmtiles` is downloaded on every SW install — but the app fetches it via Range requests at the versioned `?v=<hash>` URL (commit `8a8954e`), which misses the no-query precache key, so the precached copy is never served. Could NOT be cleanly excluded with this `@serwist/next` version: `exclude` only filters webpack assets (not public files); `manifestTransforms` is bypassed by `additionalPrecacheEntries` (how public files are added); `globPublicPatterns` uses the `glob` package (no inline `!` negation, no exposed `ignore`). Options for a dedicated pass: provide a full `additionalPrecacheEntries` list minus the tiles, upgrade serwist, or a custom manifest step.
- **[MED] Offline base-map tiles don't actually work** (despite the `sw.ts` comment claiming "CacheFirst so the map renders offline"): the runtime `mapsake-tiles` CacheFirst route can't store the 206 Range responses (no `cacheWillUpdate` permitting non-200), so the cache stays empty and offline = blank map. A real fix needs to cache a full 200 once and serve ranges from it via `RangeRequestsPlugin` (Story 4.6 gap). The `?v=` versioning fix makes ONLINE rendering correct + deploy-safe; offline is unchanged (was already broken).
- **[LOW] Map label glyphs 404 from the demo server:** `features/map/style.ts:24` `GLYPHS` points at `demotiles.maplibre.org/font/Open Sans Regular/...` which 404s in prod (~150 console errors per load). Non-fatal — MapLibre renders codepoints locally and CJK uses `localIdeographFontFamily`. The code comment already flags "prod should self-host Nunito Sans PBFs." Self-host the Latin glyph PBFs under `/public` to silence it.
- **[INFO] Returning-user sign-in completed (commit `8a7e9da`):** Google → `signInWithOAuth` (true sign-in + sign-up; a new Google user doesn't carry the anon map — email links/preserves it; Google carry-over is the deferred 2-8 merge). Email auto-sends the sign-in magic link on `email_exists` (no second tap). Network-level email enumeration is still possible (the `updateUser` 200-vs-422 differential) — close it with Supabase's email-enumeration-protection setting; the UI itself leaks nothing.
