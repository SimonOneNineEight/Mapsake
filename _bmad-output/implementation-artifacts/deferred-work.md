...
- **OAuth error feedback unconsumed + already-linked copy is generic** — `app/auth/callback/route.ts` redirects failures to `/?auth_error=oauth` but nothing reads/renders that param (identical to 2-1's accepted `/?auth_error=link`), so an OAuth failure lands the user calmly on the map with no message. Separately, `account-sheet.tsx` collapses every `linkIdentity` error into one "Google 登入暫時無法使用，請稍後再試" — misleading for the *permanent* already-linked / cross-device case (it's not transient). Per spec the already-linked resolution is explicitly Story 2-3/2-4; surface a real auth_error message (covers both `=link` and `=oauth`) together with that work.
  - **Manual-verify finding (2026-06-25):** when the linked email is already registered, Supabase returns to the redirect URL with the error in the **URL hash fragment** (`#error=invalid_request&error_code=email_exists&error_description=…`), NOT a query param — so the `/auth/callback` server route sees no `code` (falls through to `?auth_error=oauth`) and the fragment never reaches the server. 2-3's surfacing must parse `window.location.hash` client-side (read `error_code`, e.g. `email_exists`) to show "you already have an account with this email — sign in instead" and route into the returning-user/claim flow. This is the common case: a user who used the email magic link first, then tries Google with the same Gmail.
