...
- **⚠️ Pending dependency:** the 4 brand icons (`public/icons/icon-192.png`, `icon-512.png`, `maskable-512.png`, `public/apple-touch-icon.png`) are NOT yet in the repo — Simon to supply. The build/manifest/tests pass without them, but a real Lighthouse "installable" pass needs the actual files (dev currently 404s on `/icons/icon-192.png`).
